...
const RevenueReport = () => {
  ...
  useEffect(() => {
    ...
    windowElectron.ipcRenderer.send("get-report-revenue-summary");
    return () => {
  ...
  const handleExport = () => {
    ...
    const headers = ["Plan", "Members", "Total Paid (₹)", "Total Pending (₹)"];
    const rows = data.map((r) => [r.plan || "No Plan", r.memberCount, r.totalPaid || 0, r.totalPending || 0]);
    rows.push(["TOTAL", totalMembers, totalRevenue, totalPending]);
    windowElectron.ipcRenderer.send("export-report-csv", {
      headers,
      rows,
      defaultFilename: `revenue-snapshot-${new Date().toISOString().split("T")[0]}.csv`,
    });
  };
